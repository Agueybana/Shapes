...
def plot_spiral(ax):
    global LAST_DATA, CURRENT_COLORS, DATA_TO_UNDO, ALL_PLOTS
...
def reset(event):
    global TRACE_ENABLED, RECORD_ENABLED, DATA_POINTS_ENABLED, PLOTTING_ENABLED, LAST_DATA, CURRENT_COLORS, ALL_PLOTS
    TRACE_ENABLED = False
    RECORD_ENABLED = False
    DATA_POINTS_ENABLED = False
    PLOTTING_ENABLED = True
...
save_button_ax = plt.axes([button_positions[8], 0.01, button_width, 0.075])
save_button = Button(save_button_ax, 'SAVE')
save_button.on_clicked(save_data)

load_button_ax = plt.axes([button_positions[9], 0.01, button_width, 0.075])
load_button = Button(load_button_ax, 'LOAD')
load_button.on_clicked(load_data)

print_button_ax = plt.axes([button_positions[10], 0.01, button_width, 0.075])
