...
def undo(event):
    if DATA_TO_UNDO:
        ...
        if removed in ax.get_children():  # Check if the object is still a child of the axes
            removed.remove()
        plt.draw()
...
def delayed_save():
    # Your previous saving code
    temp_ax = plt.figure().add_axes([0,0,1,1], frame_on=False, aspect=1)
    if IS_3D:
        for plot in ALL_PLOTS:
            temp_ax.plot3D(*plot.get_data_3d(), color=plot.get_color())
    else:
        for plot in ALL_PLOTS:
            temp_ax.plot(*plot.get_data(), color=plot.get_color())
    temp_ax.axis('off')
...
def toggle_dimension(event):
    ...
    if IS_3D:
        ...
        toggle_dim_button.label.set_text('Switch to 2D')
    else:
        ax = fig.add_subplot(gs[0])
        toggle_dim_button.label.set_text('Switch to 3D')
    ax.axis('off')
    plot_spiral(ax)
    fig.canvas.draw()
...
dark_mode_check = CheckButtons(plt.axes([0.01, 0.95, 0.2, 0.05]), ['Dark Mode'], [False])
dark_mode_check.on_clicked(toggle_dark_mode)

toggle_dim_button_ax = plt.axes([0.01, 0.89, button_width, 0.05])
toggle_dim_button = Button(toggle_dim_button_ax, 'Switch to 2D')
...
# Crashes upon using the SAVE function
# Trace botton un-functional
# Position of icons bad upon window minimization
# PDF save is saving other items outside the plot
# PDF button always crashes (urgently fix this) if the image is heavy (e.g., with a lengthy ran dotted plot)
# ChatGPT version
